  ...
  return (
    <MarketingPageShell siteName={settings.site_name}>
      ...
      <section id="modules" className="mx-auto max-w-6xl px-4 pb-16 sm:px-6">
        <h2 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">Four modules, one dashboard</h2>
        ...
        <ul className="mt-8 grid gap-4 sm:grid-cols-2">
          {MODULES.map(({ icon: Icon, title, accent, desc }) => (
            <li key={title} className={cn(accentCard(accent), ui.cardPad)}>
              <div className="flex items-center gap-2">
                <Icon className={cn("h-5 w-5", accentText(accent))} aria-hidden />
                <h3 className={cn("font-display text-lg font-semibold", accentText(accent))}>{title}</h3>
              </div>
